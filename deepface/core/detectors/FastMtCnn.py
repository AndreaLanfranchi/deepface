...
class Detector(DetectorBase):
    ...
    def process(
        ...
        detection_results = self._detector.detect(img_rgb, landmarks=key_points)

        if detection_results is not None and not any(
            item is None for item in detection_results
        ):
            if len(detection_results) == 2:
                boxes, probs = detection_results
                keypoints_list = numpy.full((len(boxes), 1, 2), numpy.nan)
            elif len(detection_results) == 3:
                boxes, probs, keypoints_list = detection_results
            ...
            for box, prob, keypoints in zip(boxes, probs, keypoints_list):
                ...
                if (
                    box is None
                    or not isinstance(box, numpy.ndarray)
                    or box.shape[0] != 4
                ):
                ...
                if (
                    ...
                    if keypoints.shape[0] >= 5:
                        ...
                        points.update(
                            {"mlc": lm_point, "mrc": rm_point, "mc": cm_point}
                        )
